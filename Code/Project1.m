...
xRange = [1, 1, 1, 1]; % Range of x values for plotting window
yRange = [1, 1, 1, 1]; % Range of y values for plotting window
...
M = cell(length(c),1);
for i = 1:length(c)
    M{i} = FilledJuliaSet(phi,xRange(1),yRange(1),100,c(i),'nocolor',2);
end
...
for i = 1:length(c)
    ...
    image( [-xRange xRange], [-yRange yRange], M{i}') % create map
    axis xy
    axis equal
    ax = gca;
...
phi = @(z,c) z^2 - c;
for i = 1:length(c)
    % Once again using the Filled Julia Set function with options 
    % 'colored' to color plots based on iterations to converge to diverge
...
for i = 1:length(c)
    ...
    xlabel('\Re','Fontsize',18)
    ylabel('\Im','Fontsize',18)
    colormap(jet(max(max(M{i})))) % Set Color map range to 100
    colorbar
    hold off
    if bsave == 1 % Save plot to file if bsave = 1
...
g = @(z,n) (z^n - 1)/(n*z^(n-1));
a = linspace(-5,5,500); % set plottig window
b = linspace(-5,5,500);
